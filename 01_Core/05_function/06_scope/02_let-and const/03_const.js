...
변수의 스코프는 최대한 좁게 만든다.
변경이 발생하지 않고 읽기 전용으로 
사용하는 원시 값과 객체에는 const 키워드를 사용한다.
...
2. 전역 변수는 "생명주기가 길다". 메모리 리소스도 오래 소비하며, 
상태 변경이 가능한 시간 및 기회가 많다.
더욱이 중복 된 변수명을 통해 의도치 않은 재할당이 일어날 수도 있다.
3. "스코프 체인 상에서 종점에 존재"하므로 전역 변수의 검색속도가 가장 느리다.
4. "네임스페이스가 오염"된다. 자바스크립트는 파일이 분리 되어 있어도 
하나의 전역 스코프를 공유하므로 예상치 못한 결과를 가져올 수 있다.
=> 변수의 스코프는 좁을수록 좋다.
 */
